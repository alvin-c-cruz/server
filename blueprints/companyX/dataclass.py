...
class CompanyX(Voucher):
    	...
    	if filter:
    		...
    		sql = f"""
    			SELECT 
                    ...
                    tbl_companyx.check_notes,
                    tbl_companyx.check_number,
    				tbl_companyx.description
    			FROM tbl_companyx
    			INNER JOIN tbl_vendor ON tbl_vendor.id = tbl_companyx.vendor_id
    			WHERE {", ".join(clause)}
    		"""
    ...
    def range(self, date_from, date_to):
        if True:
            sql = f"""
                ...
                FROM tbl_companyx
                INNER JOIN tbl_vendor ON tbl_vendor.id = tbl_companyx.vendor_id
                WHERE tbl_companyx.record_date>=? AND tbl_companyx.record_date<=?
            """
    ...
    def is_validated(self):
        if self.id:
            if self.db.execute('SELECT COUNT(*) FROM tbl_companyx WHERE cd_num=? AND id!=?;', (self.cd_num, self.id)).fetchone()[0]:
                flash("CD Number is already in use.")
                return False
            if self.db.execute('SELECT COUNT(*) FROM tbl_companyx WHERE check_number=? AND id!=?;', (self.check_number, self.id)).fetchone()[0]:
                flash("Check Number is already in use.")
                return False
        else:
            if self.db.execute('SELECT COUNT(*) FROM tbl_companyx WHERE cd_num=?;', (self.cd_num, )).fetchone()[0]:
                flash("CD Number is already in use.")
                return False
