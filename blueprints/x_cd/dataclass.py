...
class XCD(Voucher):
    ...
    def range(self, date_from, date_to):
        if True:
            sql = f"""
                ...
                FROM tbl_x_cd
                INNER JOIN tbl_vendor ON tbl_vendor.id = tbl_x_cd.vendor_id
                WHERE tbl_x_cd.record_date>=? AND tbl_x_cd.record_date<=?
            """
    ...
    def is_validated(self):
        if self.id:
            if self.db.execute('SELECT COUNT(*) FROM tbl_x_cd WHERE cd_num=? AND id!=?;', (self.cd_num, self.id)).fetchone()[0]:
                flash("CD Number is already in use.")
                return False
            if self.db.execute('SELECT COUNT(*) FROM tbl_x_cd WHERE check_number=? AND id!=?;', (self.check_number, self.id)).fetchone()[0]:
                flash("Check Number is already in use.")
                return False
        else:
            if self.db.execute('SELECT COUNT(*) FROM tbl_x_cd WHERE cd_num=?;', (self.cd_num, )).fetchone()[0]:
                flash("CD Number is already in use.")
                return False
            if self.db.execute('SELECT COUNT(*) FROM tbl_x_cd WHERE check_number=?;', (self.check_number, )).fetchone()[0]:
                flash("Check Number is already in use.")
                return False
...
def get_cd(date_from, date_to):
    ...
    sql = f"""SELECT
            ...
            FROM tbl_x_cd_entry as entry

            INNER JOIN tbl_x_cd as cd ON entry.x_cd_id = cd.id
...
class Create_File:
    ...
    def sheet_transactions(self, wb):
        ...
        row_num += 1
        cell = ws[f'A{row_num}']
        cell.value = "Company X Disbursement Journal"
